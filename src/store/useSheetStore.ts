...
const initialCharacter: CharacterDB = {
  ...
  stats_current: { pv: 20, pe: 2, san: 12, conditions: [], is_dying: false, is_stable: true },
...
function calculateMaxStats(char: CharacterDB) {
    ...
    if (!cls) return { ...char.stats_max, slots: char.inventory_meta.load_limit };
...
export const useSheetStore = create<SheetStore>((set, get) => ({
  ...
  equipItem: (item: ItemDB) => { 
    set((s) => ({ items: [...s.items, item as any] }));
    get().recalculateDerivedStats();
