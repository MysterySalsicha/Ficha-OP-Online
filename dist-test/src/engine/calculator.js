...
exports.getProgressionLimits = getProgressionLimits;
exports.calculateMaxStats = calculateMaxStats;
exports.calculateSkillBonus = calculateSkillBonus;
exports.recalculateCharacter = recalculateCharacter;
...
function getProgressionLimits(nex) {
    ...
    if (nex < 5)
        return { maxAttribute: 3, maxSkill: 'destreinado' };
    if (nex < 20)
        return { maxAttribute: 3, maxSkill: 'treinado' };
    if (nex < 50)
        return { maxAttribute: 4, maxSkill: 'treinado' };
    if (nex < 80)
        return { maxAttribute: 5, maxSkill: 'veterano' };
...
function calculateSkillBonus(char, skillName, attributeName) {
    const trainingLevel = char.skills[skillName] || 0;
    return trainingLevel;
}
function recalculateCharacter(char, items) {
    const newChar = { ...char };
    // Patente
    if (char.class === 'sobrevivente' || char.nex < 5)
        newChar.patente = 'Mundano';
    else if (char.nex < 10)
        newChar.patente = 'Recruta';
    else if (char.nex < 25)
        newChar.patente = 'Operador';
    else if (char.nex < 50)
        newChar.patente = 'Agente Especial';
    else if (char.nex < 85)
        newChar.patente = 'Oficial de Operações';
    else
        newChar.patente = 'Agente de Elite';
